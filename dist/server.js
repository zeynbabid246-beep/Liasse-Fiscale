...
import { initPostgresDatabase, saveDepositDb, saveDepositFileDb, saveDeclarationDetailsDb, logAuditDb } from './src/db/postgres.js';
// Patch SchemaModel pour la résolution robuste des préfixes QName (ex: lf:T_NombrePositif15)
const origResolveSimple = SchemaModel.prototype.resolveSimpleType;
SchemaModel.prototype.resolveSimpleType = function (qname) {
    if (!qname)
        return null;
...
const origResolveComplex = SchemaModel.prototype.resolveComplexType;
SchemaModel.prototype.resolveComplexType = function (qname) {
    if (!qname)
        return null;
...
function findContribuableByInput(input) {
    if (!input)
        return null;
    ...
    const byMatricule = contribuablesDb.find(c => c.matriculeFiscal.toUpperCase() === clean ||
...
// -------------------------------------------------------------
function getEtatsRequis(categorie, modeleF6004) {
...
function getOfficialXsdSchema(codeDocument) {
...
function getBusinessRules(codeDocument) {
...
function formatXsdErrorMessage(rawMessage, elemName) {
    if (!rawMessage)
        return 'Erreur de validation de schéma XSD.';
...
function validerXmlComplet(codeDocument, xmlString, matriculeAttendu, exerciceAttendu) {
    const erreurs = [];
    ...
    const detailsFlat = {};
    const root = doc.root;
    const enteteEl = root?.childElements?.find((c) => c.localName === 'Entete');
    const detailsEl = root?.childElements?.find((c) => c.localName === 'Details');
    if (enteteEl) {
        const matriculeDeclarant = enteteEl.childElements?.find((c) => c.localName === 'MatriculeFiscalDeclarant')?.textContent?.trim() || '';
        const exerciceStr = enteteEl.childElements?.find((c) => c.localName === 'Exercice')?.textContent?.trim() || '';
        const exerciceXml = parseInt(exerciceStr, 10);
        const acteDeDepot = enteteEl.childElements?.find((c) => c.localName === 'ActeDeDepot')?.textContent?.trim() || '';
        const natureDepot = enteteEl.childElements?.find((c) => c.localName === 'NatureDepot')?.textContent?.trim() || '';
        if (matriculeAttendu && matriculeDeclarant) {
            ...
            if (shortFound !== shortExpected && !cleanFound.startsWith(cleanExpected) && !cleanExpected.startsWith(cleanFound)) {
                erreurs.push({
                    ...
                    ligne: enteteEl.childElements?.find((c) => c.localName === 'MatriculeFiscalDeclarant')?.sourceLine || null,
        ...
        if (exerciceAttendu && exerciceXml && exerciceXml !== exerciceAttendu) {
            erreurs.push({
                source: 'Structurelle',
                champ: 'Exercice',
                ligne: enteteEl.childElements?.find((c) => c.localName === 'Exercice')?.sourceLine || null,
        ...
        if (acteDeDepot && !['0', '1', '2'].includes(acteDeDepot)) {
            erreurs.push({
                source: 'Structurelle',
                champ: 'ActeDeDepot',
                ligne: enteteEl.childElements?.find((c) => c.localName === 'ActeDeDepot')?.sourceLine || null,
        ...
        if (natureDepot && !['D', 'P'].includes(natureDepot.toUpperCase())) {
            erreurs.push({
                source: 'Structurelle',
                champ: 'NatureDepot',
                ligne: enteteEl.childElements?.find((c) => c.localName === 'NatureDepot')?.sourceLine || null,
        ...
        for (const rule of rulesList) {
            ...
            if (detailsFlat[target] !== undefined || atLeastOneOperandFound) {
                ...
                if (diff > 0.01) {
                    ...
                    const formulaDesc = rule.formulaRaw || (Array.isArray(rule.operands) ? rule.operands.map((o) => typeof o === 'string' ? o : `${o.sign === '-' ? '- ' : '+ '}${o.code}`).join(' ') : '');
                    erreurs.push({
                        source: 'RegleMetier',
                        champ: target,
                        ligne: null,
...
app.get('/api/auth/me', (req, res) => {
    ...
    jwt.verify(token, JWT_SECRET, (err, user) => {
        if (err)
            return res.status(403).json({ message: "Jeton expiré ou invalide" });
        res.json(user);
    });
});
// 2. Contribuables
app.get('/api/contribuables/search', (req, res) => {
...
app.get('/api/contribuables/:matricule', (req, res) => {
    ...
    if (!found) {
        found = contribuablesDb.find(c => c.matriculeFiscal.toUpperCase() === clean ||
...
app.get('/api/liasses/etats-requis', (req, res) => {
...
app.get('/api/liasses/en-cours', (req, res) => {
...
app.get('/api/liasses/:id', (req, res) => {
...
app.post('/api/liasses', (req, res) => {
    ...
    if (dtype === 'Provisoire') {
        const existingDefinitive = liassesDb.find(l => (l.contribuableId === contrib.id || l.matriculeFiscal === contrib.matriculeFiscal) &&
            l.exercice === ex &&
            l.nature === nat &&
    ...
    if (nat === 'Rectification' || nat === 'Rectificative') {
        const hasSpontane = liassesDb.find(l => (l.contribuableId === contrib.id || l.matriculeFiscal === contrib.matriculeFiscal) &&
            l.exercice === ex &&
    ...
    let existing = liassesDb.find(l => (l.contribuableId === contrib.id || l.matriculeFiscal === contrib.matriculeFiscal) &&
        l.exercice === ex &&
        l.statut === 'EnSaisie');
    if (existing) {
...
// Vérifier Liasse
app.post('/api/liasses/:id/verifier', (req, res) => {
    const id = parseInt(String(req.params.id), 10);
    const liasse = liassesDb.find(l => l.id === id && l.statut !== 'Supprimee');
    if (!liasse) {
        return res.status(404).json({ message: "Liasse introuvable." });
...
app.delete('/api/liasses/:id', (req, res) => {
    const id = parseInt(String(req.params.id), 10);
...
app.post('/api/liasses/:id/documents/:codeDocument', upload.any(), (req, res) => {
    ...
    try {
        ...
        logAuditDb(liasse.matriculeFiscal, doc.statut === 'Valide' ? 'VALIDATION_XML_SUCCES' : 'VALIDATION_XML_ERREUR', `Document ${codeDocument} : ${result.erreurs.length} anomalie(s)`, `LIASSE-${liasse.id}`);
    }
    catch (err) {
...
app.delete('/api/liasses/:id/documents/:codeDocument', (req, res) => {
    const liasseId = parseInt(String(req.params.id), 10);
...
app.get('/api/liasses/:id/documents/:codeDocument/download', optionalToken, (req, res) => {
    const liasseId = parseInt(String(req.params.id), 10);
    const code = String(req.params.codeDocument || '').toUpperCase();
    const liasse = liassesDb.find(l => l.id === liasseId);
    if (!liasse)
...
app.get('/api/liasses/:id/documents/:codeDocument/html', optionalToken, (req, res) => {
    const liasseId = parseInt(String(req.params.id), 10);
    const code = String(req.params.codeDocument || '').toUpperCase();
    const liasse = liassesDb.find(l => l.id === liasseId);
    if (!liasse)
        return res.status(404).send("Liasse introuvable.");
    const doc = liasse.documents.find(d => d.codeDocument.toUpperCase() === code);
    if (!doc)
...
function executerDepot(liasse, observation, signature) {
    ...
    try {
        ...
        logAuditDb(deposit.matriculeFiscal, 'DEPOT_SOUMIS', `Liasse fiscale déposée sous la référence ${deposit.reference}`, deposit.reference);
    }
    catch (err) {
        // Non-bloquant
    }
...
app.post('/api/liasses/:id/deposit', (req, res) => {
    const liasseId = parseInt(String(req.params.id), 10);
...
app.get('/api/admin/deposits', optionalToken, (req, res) => {
...
app.post('/api/admin/deposits/:reference/validate', optionalToken, (req, res) => {
...
app.post('/api/admin/deposits/:reference/reject', optionalToken, (req, res) => {
    const ref = String(req.params.reference || '').trim();
...
app.get('/api/deposits', optionalToken, (req, res) => {
...
app.get(['/api/deposits/:reference', '/api/tracking/:reference'], (req, res) => {
    const ref = String(req.params.reference || '').trim();
    const deposit = depositsDb.find(d => d.reference.toUpperCase() === ref.toUpperCase());
    if (!deposit) {
...
app.get('/api/deposits/:reference/documents/:codeDocument/download', optionalToken, (req, res) => {
    const ref = String(req.params.reference || '').trim();
    const code = String(req.params.codeDocument || '').toUpperCase();
    const deposit = depositsDb.find(d => d.reference.toUpperCase() === ref.toUpperCase());
    if (!deposit)
...
app.get('/api/deposits/:reference/documents/:codeDocument/view', optionalToken, (req, res) => {
    const ref = String(req.params.reference || '').trim();
    const code = String(req.params.codeDocument || '').toUpperCase();
    const deposit = depositsDb.find(d => d.reference.toUpperCase() === ref.toUpperCase());
    if (!deposit)
...
app.get(['/api/deposits/:reference/receipt', '/api/tracking/:reference/receipt/pdf'], optionalToken, (req, res) => {
    const ref = String(req.params.reference || '').trim();
    const deposit = depositsDb.find(d => d.reference.toUpperCase() === ref.toUpperCase());
    if (!deposit)
        return res.status(404).send("Dépôt introuvable.");
